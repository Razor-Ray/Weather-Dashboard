...
var searchBttn = $(".search")
var searchQuery = $(".searchQuery")
var inputCityName = $("#inputCityName")
var searchText = inputCityName.val()
var weatherForecastDisplay = $(".weatherForecastDisplay")
var weatherOnDay = $(".weatherOnDay")
var openWeatherAPI = 'acbf05eb3db32edd9ddb78763052132e'
...
function getWeatherData(){
    var url = "https://api.openweathermap.org/data/2.5/weather?q="+searchText+"&appid="+openWeatherAPI
    fetch(url)
    .then(function(response){
        ...
        if(data.message === "city not found"){
            ...
            var tempFahrenheit = parseFloat(((tempKelvin-273.15)*9/5)+32).toFixed(2)+'&#8457'
            var date = moment.unix(data.dt).format("(Do MMMM, YYYY)")
            var cityName = data.name
            var speed = parseFloat((2.236937*data.wind.speed)).toFixed(2)+"MPH"
            var humidity = data.main.humidity+"%"
            var icon = data.weather[0].icon
            var iconDes = data.weather[0].description
            var weatherImg =  'https://openweathermap.org/img/wn/'+icon+'@2x.png'
            weatherOnDay.append('<h5 class="bg-info">'+cityName+" "+date+'<img src="'+weatherImg+'"><sup>'+iconDes+'</sup></h5><p>Temp: '+tempFahrenheit+'</p><p>Wind: '+speed+'</p><p>Humidity: '+humidity+'</p><p id = "lat">'+locationLatitiude+'</p><p id = "long">'+locationLongitude+'</p>')
...
var fiveDayForecast = function(){
    ...
    var lat = $('#lat').html()
    var long = $('#long').html()
    var url = 'https://api.openweathermap.org/data/2.5/onecall?lat='+lat+'&lon='+long+'&appid='+openWeatherAPI
    fetch(url)
    .then(function (response){
        return response.json();
    }).then(function(data){
        ...
        for(i=1;i<6;i++){
            ...
            var tempFahrenheit =  parseFloat(((tempKelvin-273.15)*9/5)+32).toFixed(2)+'&#8457'
            var wind = parseFloat(2.236937*data.daily[i].wind_speed).toFixed(2)+" MPH"
            var humidity = data.daily[i].humidity+"%"
            var icon = data.daily[i].weather[0].icon
            var weatherImg =  'https://openweathermap.org/img/wn/'+icon+'@2x.png'
            fiveDayWeather.append('<div class="custom-card text-white bg-dark p-1"><h5 class="card-title">'+forecastDate+'</h5><img src="'+weatherImg+'"><h6>Temp: '+tempFahrenheit+'</h6><h6>Wind: '+wind+'</h6><h6>Humidity: '+humidity+'</h6></div><div class="p-1"></div>')
        }
    })
}
...
function getSearchHistory(){
    ...
    if (storedList !== null) {
        for(i=0; i < storedList.length; i++){
            $(".history").append('<button type="button" class="col-12 btn btn-secondary historyBttn" role="button" id ="'+storedList[i]+'"onclick="historyBttnClick(event)">'+ storedList[i] +'</button><div class="p-1"></div>' )
        }        
    }
}
